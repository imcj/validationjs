...
var FieldError = result.FieldError,
    FieldValidationResult = result.FieldValidationResult
...
module.exports.IntegerConstraint = function() {
    ...
    this.validate = function(fieldName, fieldLabel, data) {
        var value = clean(data, fieldName)
        var fieldValidationResult = new FieldValidationResult(fieldName,
            fieldLabel, this.name, data[fieldName])
        var yes = isInt(value)
        if (!yes) {
            var error = new FieldError(fieldName, fieldLabel, this.name,
                sprintf(this.message, {name: fieldLabel}))

            fieldValidationResult.addError(error)
        }
